...
(sexml:with-compiletime-active-layers (sexml:standard-sexml sexml:xml-doctype)
  (sexml:support-dtd (merge-pathnames "html5.dtd" (asdf:system-source-directory "sexml"))
		     :<))
...
(defun slug (strg)
  (substitute #\- #\Space
	      (string-downcase
	       (string-trim '(#\Space #\Tab #\Newline) strg))))
...
(defun html-frame (title body)
  (<:html
   (<:head (<:title title))
   (<:body (<:a :href (genurl 'home) (<:h1 title))
	   body)))

(defun render-post (post)
  (list (<:div
	 (<:h2 (<:a :href (genurl 'post :id (position post *posts* :test #'equal))
		    (getf post :title)))
...
(defun add-post-form ()
  (html-frame
   ...
   (<:form :action (genurl 'add/post) :method "post"
	   ...
	   "Content:" (<:br)
	   (<:textarea :name "content" :rows 30 :cols 200) (<:br)
